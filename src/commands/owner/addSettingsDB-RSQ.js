...
module.exports = {
    ...
    callback: async (client, interaction) => {
        try {
            ...
            const existingGuildSettings = await SettingBattalion.findOne({ guildId: interaction.guild.id });
        
            if (existingGuildSettings) {
                // Если запись уже существует, удаляем ее
                await SettingBattalion.findOneAndDelete({ guildId: interaction.guild.id });
            }

            // Создаем новую запись GuildSettings для новой гильдии
            const newGuildSettings = new SettingBattalion({
